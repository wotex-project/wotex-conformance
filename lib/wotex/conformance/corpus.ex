defmodule Wotex.Conformance.Corpus do
  ...
  defp build(id, revision, vectors) do
    ...
    cond do
      sorted == [] ->
        {:error, Error.new(:empty_corpus, "corpus must contain at least one vector")}

      length(sorted) > @max_vectors ->
        {:error,
         Error.new(:limit_exceeded, "corpus exceeds its vector limit",
           details: %{"max_vectors" => @max_vectors}
         )}

      duplicate_ids?(sorted) ->
        {:error, Error.new(:duplicate_vector, "corpus contains duplicate vector IDs")}
    ...
    |> Enum.reduce_while({:ok, []}, fn value, {:ok, vectors} ->
      case value do
        ...
        input when is_map(input) ->
          case Vector.new(input) do
            {:ok, vector} -> {:cont, {:ok, [vector | vectors]}}
            {:error, error} -> {:halt, {:error, error}}
          end

        _value ->
          {:halt, {:error, Error.new(:invalid_type, "corpus vectors must be objects")}}
      end
    end)
    |> then(fn
  ...
  defp construct_vectors(_values),
    do: {:error, Error.new(:invalid_value, "vectors must be a bounded list")}

  defp validate_schema_version(%{"schema_version" => @schema_version}), do: :ok

  defp validate_schema_version(_manifest) do
    {:error, Error.new(:unsupported_schema_version, "corpus schema_version is not supported")}
  end
    ...
    |> then(fn
      {:ok, valid} ->
        ...
        if duplicate_entry_files?(valid) do
          {:error,
           Error.new(:duplicate_vector_file, "corpus manifest contains duplicate vector files")}
        else
          {:ok, valid}
        end
  ...
  defp validate_entries(_entries),
    do: {:error, Error.new(:invalid_manifest, "manifest vectors must be a non-empty bounded list")}
  ...
  defp validate_entry(_entry),
    do: {:error, Error.new(:invalid_manifest_entry, "vector manifest entry is invalid")}
  ...
  defp validate_filename(file) when is_binary(file) do
    if Path.basename(file) == file and String.match?(file, ~r/^[a-z0-9][a-z0-9._-]*\.json$/) do
      ...
      {:error, Error.new(:invalid_vector_filename, "vector filename must be a local JSON basename")}
    end
  end

  defp validate_filename(_file),
    do: {:error, Error.new(:invalid_vector_filename, "vector filename must be a string")}
  ...
  defp compare_entry(entry, input, vector) do
    cond do
      entry["id"] != vector.id ->
        {:error,
         Error.new(:vector_identity_mismatch, "vector ID does not match its manifest entry")}

      entry["digest"] != vector.digest ->
        {:error,
         Error.new(:vector_digest_mismatch, "vector digest does not match its manifest entry")}

      Map.get(input, "digest") != vector.digest ->
        {:error,
         Error.new(:vector_digest_mismatch, "vector embedded digest does not match its content")}
  ...
  defp validate_manifest_digest(manifest, actual_digest) do
    case Map.get(manifest, "digest") do
      ...
      _digest ->
        {:error, Error.new(:corpus_digest_mismatch, "corpus digest does not match its manifest")}
    end
  end
  ...
  defp read_json_file(path, max_bytes) do
    with {:ok, %File.Stat{type: :regular, size: size}} when size <= max_bytes <- File.lstat(path),
      ...
      {:ok, %File.Stat{type: :regular}} ->
        {:error, Error.new(:limit_exceeded, "corpus file exceeds its byte limit")}

      {:ok, _stat} ->
        {:error, Error.new(:invalid_corpus_file, "corpus entry must be a regular file")}

      {:error, %Jason.DecodeError{}} ->
        {:error, Error.new(:invalid_json, "corpus file contains invalid JSON")}

      {:error, _reason} ->
        {:error, Error.new(:corpus_unreadable, "corpus file could not be read")}
    end
  end
